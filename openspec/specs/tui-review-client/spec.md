...
- **THEN** the system SHALL display the PR metadata, full description, an action-state line (awaiting the user's action, waiting on author, or waiting on others) and, when available, the reason chips that produced it (e.g., directly requested, re-review due, respond to review)
